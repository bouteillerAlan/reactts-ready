    ...
    return (
      <ul className="collapsible">
        {this.props.datas.map((data, index) => (
          <li key={data.header}>
            <div
              className={`collapsible_header ${this.props.datas.length === index + 1 ? '' : 'special_border close'}`}>
              <div className="text">{data.header}</div>
              <div className="icon close" onClick={(e) => {
                this.handle(e)
              }}></div>
            </div>
            <div className={`collapsible_body close ${this.props.datas.length === index + 1 ? 'last_border' : ''}`}>
              <span><Interwave content={data.text}/></span></div>
          </li>
        ))}
      </ul>
    );
